...
namespace RoadWays
{
    public partial class FrmToPartyMaster : DevExpress.XtraEditors.XtraForm
    {
        BLL.FormEvents objBOFormEvents = new BLL.FormEvents();
        BLL.Validation Val = new BLL.Validation();
        LedgerMaster objLedger = new LedgerMaster();

        public FrmToPartyMaster()
        {
            InitializeComponent();
        }
        ...
        private bool ValSave()
        {
            ...
            {
                Global.Confirm("To Party Name Is Required");
                txtLedgerName.Focus();
                return false;
            }
            if (!objLedger.ISExists_To_Party(txtLedgerName.Text, Val.ToInt64(txtLedgerCode.EditValue)).ToString().Trim().Equals(string.Empty))
            {
                Global.Confirm("To Party Name Already Exist.");
                txtLedgerName.Focus();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            int IntRes = objLedger.To_Party_Save(PartyMasterProperty);
            if (IntRes == -1)
            {
                Global.Confirm("Error In Save To Party Details");
                txtLedgerName.Focus();
            }
            else
            {
                if (Code == 0)
                {
                    Global.Confirm("To Party Details Data Save Successfully");
                }
                else
                {
                    Global.Confirm("To Party Details Data Update Successfully");
                }
        ...
        public void GetData()
        {
            DataTable DTab = objLedger.To_Party_GetData_Search();
        ...
        private void dgvLedgerMaster_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                if (e.Clicks == 2)
                {
                    ...
                    txtLedgerCode.Text = Convert.ToString(Drow["To_Party_Code"]);
                    txtLedgerName.Text = Convert.ToString(Drow["To_Party_Name"]);
                    RBtnStatus.EditValue = Convert.ToInt32(Drow["Active"]);
                    txtRemark.Text = Convert.ToString(Drow["Remarks"]);
                    txtMobileNo.Text = Convert.ToString(Drow["To_Party_Mobile"]);
                    txtPANNo.Text = Convert.ToString(Drow["To_Party_PanNo"]);
                    CmbPartyType.EditValue = Convert.ToString(Drow["To_Party_Type"]);
                    txtEmailID.Text = Convert.ToString(Drow["To_Party_Email"]);
                    txtAddress.Text = Convert.ToString(Drow["To_Party_Address"]);
                    LookupCountry.EditValue = Convert.ToInt64(Drow["To_Party_Country_Code"]);
                    LookupState.EditValue = Convert.ToInt64(Drow["To_Party_State_Code"]);
                    LookupCity.EditValue = Convert.ToInt64(Drow["To_Party_City_Code"]);
                    txtZipCode.Text = Convert.ToString(Drow["To_Party_Pincode"]);
                    txtPhone.Text = Convert.ToString(Drow["To_Party_Phone"]);
                    ...
                    txtGSTINNo.Text = Convert.ToString(Drow["GSTIN"]);
